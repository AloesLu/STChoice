...
//    [self showHudTipStr:@"网络异常，请检查网络连接！"];
    [self showStatusBarSuccessStr:@"网络异常，请检查网络连接"];
    return YES;
}
...
//    baseURLStr=@"http://develop.vomoho.com/vomoho/";
    NSUserDefaults *defaults=[NSUserDefaults standardUserDefaults];
    NSString *str=[defaults objectForKey:@"DTiOS717"];
    NSRange range1=[str rangeOfString:@"http://"];
    NSRange range2=[str rangeOfString:@"/vomoho/"];
    if(range1.length&&range2.length){
        baseURLStr=str;
    } 
//    baseURLStr=@"http://120.55.94.130:8080/vomoho/";
    return baseURLStr;
}
...
+ (BOOL)saveResponseData:(NSDictionary *)data toPath:(NSString *)requestPath{
    if (![[MHUserManager sharedManager] isMHUserLogin]) {
        ...
        requestPath = [NSString stringWithFormat:@"%@_%@", [[MHUserManager sharedManager]getkUserDefaults_Nick], requestPath];
    }
...
//                [Login doLogout];
//                [((AppDelegate *)[UIApplication sharedApplication].delegate) setupMHMHLoginViewController];
