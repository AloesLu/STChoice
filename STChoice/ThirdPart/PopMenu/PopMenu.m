...
- (void)showButtons {
    ...
    for (int index = 0; index < items.count; index ++) {
        ...
        CGFloat menuButtonHeight=MenuButtonHeight;
        if (kDevice_Is_iPhone6Plus) {
            menuButtonHeight=120;
        }
        if (kDevice_Is_iPhone6) {
            menuButtonHeight=105;
        }
...
//        if(index==9){
//            menuButton = [[MenuButton alloc] initWithFrame:CGRectMake(kScreen_Width/2-30, kScreen_Height-104, 60, 60) menuItem:menuItem];
//        }
...
//    MenuItem *item=[MenuItem itemWithTitle:nil iconName:@"房产装修" index:9];
//    MenuButton *clickButton = [[MenuButton alloc] initWithFrame:CGRectMake(kScreen_Width/2-30, kScreen_Height-104, 60, 60) menuItem:item];;
//    clickButton.tag = 109;
//    clickButton.didSelctedItemCompleted = ^(MenuItem *menuItem) {
//        weakSelf.selectedItem = menuItem;
//        [weakSelf dismissMenu];
//    };
